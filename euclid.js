...
var meter_changed = false;

function rr_on_meter_change() {
    meter_changed = !meter_changed;
    post("meter_changed:", meter_changed, "\n");
}

var maxVoices = 6;

function rr_on_error(err) {
    // Set num voices active
    var numVoicePossibilities = [1, 2, 3, 4, 5, 6];
    var numVoiceQuantizations = [0.0, 0.3, 0.4, 0.6, 0.7, 0.8];
    var numVoiceQuantizedError = quantize(err, numVoiceQuantizations);
    var numVoices = numVoicePossibilities[numVoiceQuantizations.indexOf(numVoiceQuantizedError)];
    for (var i = numVoices; i < maxVoices; i++) pulse(i, 0);

    if (!meter_changed) {  // 4/4 section
        // Set steps to 16
        for (var i = 0; i < maxVoices; i++) step(i, 16);

        // Set num pulses for each voice
        var numPulseMultiplePossibilities = [2, 4, 3, 5, 7];
        var numPulseQuantizations = [0.0, 0.2, 0.4, 0.6, 0.7];
        var numPulseQuantizedError = quantize(err, numPulseQuantizations);
        var numPulseMultiple = numPulseMultiplePossibilities[numPulseQuantizations.indexOf(numPulseQuantizedError)];
        for (var i = 0; i < numVoices; i++) {
            var numPulses = numPulseMultiple * Math.round(Math.random() * Math.floor(16 / numPulseMultiple));

            // Make sure there's pulses if err is high enough
            if (err > 0.5 && numPulses == 0) numPulses = numPulseMultiple;

            // Make even more potentially irregular if err is high enough
            if (err > 0.4) {
                if (numPulses < 15 && numPulses > 1) {
                    numPulses += Math.round(Math.random() + (-1 * Math.random()));
                }
            }
            pulse(i, numPulses);
        }

        // Set offset for each voice
        var offetMaxPossibilities = [0, 1, 2, 3, 5];
        var offetMaxQuantizations = [0.0, 0.2, 0.4, 0.6, 0.7];
        var offetMaxQuantizedError = quantize(err, offetMaxQuantizations);
        var maxOffset = offetMaxPossibilities[offetMaxQuantizations.indexOf(offetMaxQuantizedError)];
        for (var i = 0; i < numVoices; i++) {
            var voiceOffset = Math.floor(Math.random() * maxOffset);
            offset(i, voiceOffset);
        }


        // Set accents for each voice
        var maxAccentPossibilities = [0, 2, 4, 5, 6, 7];
        var maxAccentQuantizations = [0.0, 0.3, 0.4, 0.6, 0.7, 0.8];
        var maxAccentQuantizedError = quantize(err, maxAccentQuantizations);
        var maxAccent = maxAccentPossibilities[maxAccentQuantizations.indexOf(maxAccentQuantizedError)];
        for (var i = 0; i < numVoices; i++) {
            var voiceAccent = Math.round(maxAccent * Math.random());
            if (voiceAccent > pulses[i]) voiceAccent = pulses[i];
            accent(i, voiceAccent);
        }

        // Set accent offsets for each voice
        var maxAccentOffsetPossibilities = [0, 1, 2];
        var maxAccentOffsetQuantizations = [0.0, 0.5, 0.8];
        var maxAccentOffsetQuantizedError = quantize(err, maxAccentOffsetQuantizations);
        var maxAccentOffset = maxAccentOffsetPossibilities[maxAccentOffsetQuantizations.indexOf(maxAccentOffsetQuantizedError)];
        for (var i = 0; i < numVoices; i++) {
            var voiceAccentOffset = Math.round(maxAccentOffset * Math.random());
            if (voiceAccentOffset > accents[i] || voiceAccentOffset > pulses[i]) voiceAccentOffset = -1;
            accent_offset(i, voiceAccentOffset);
        }
    } else {  // 5/8 section
        // Set steps to 20
        for (var i = 0; i < maxVoices; i++) step(i, 20);

        // Set num pulses for each voice
        var numPulseMaxPossibilities = [1, 5, 15, 20];
        var numPulseMaxQuantizations = [0.0, 0.2, 0.7, 0.8];
        var numPulseMaxQuantizedError = quantize(err, numPulseMaxQuantizations);
        var numPulseMax = numPulseMaxPossibilities[numPulseMaxQuantizations.indexOf(numPulseMaxQuantizedError)];
        for (var i = 0; i < numVoices; i++) {
            var numPulses;
            if (numPulseMax == 1) numPulses = 1;
            else {
                var numPulseMaxMultiple = numPulseMax / 5;
                numPulses = 5 * Math.round(Math.random() * numPulseMaxMultiple);
                if (numPulses == 0) numPulses = 1;
            }
            pulse(i, numPulses);
        }

        // Set offset for each voice
        var offetMaxPossibilities = [0, 1, 2, 4, 5];
        var offetMaxQuantizations = [0.0, 0.3, 0.4, 0.6, 0.7];
        var offetMaxQuantizedError = quantize(err, offetMaxQuantizations);
        var maxOffset = offetMaxPossibilities[offetMaxQuantizations.indexOf(offetMaxQuantizedError)];
        for (var i = 0; i < numVoices; i++) {
            var voiceOffset = Math.round(Math.random() * maxOffset);
            offset(i, voiceOffset);
        }
...
var stateArr = [steps, pulses, accents, offsets, aoffsets];
var funcArr = [step, pulse, accent, offset, accent_offset];

function quantize(val, quant) {
    var mids = []
    for (var i = 0; i < quant.length - 1; i++) {
        mids[i] = (quant[i] + quant[i + 1]) / 2.0;
    }
    var ind = bisectRight(mids, val);
    return quant[ind];
}
